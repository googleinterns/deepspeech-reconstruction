...
def sample_from_mfcc(mfcc, max_length, random=True):
    if mfcc.shape[0] >= max_length:
        r = choice(range(0, len(mfcc) - max_length + 1)) if random else 0
...
def sample_from_mfcc_file(utterance_file, max_length, random=True):
    mfcc = np.load(utterance_file)
    return sample_from_mfcc(mfcc, max_length, random)
...
class LazyTripletBatcher:
    ...
    def get_random_batch(self, batch_size, is_test=False):
        ...
        for anchor_speaker in anchor_speakers:
            ...
            pos_utterances = np.random.choice(sp_to_utt[anchor_speaker][:5], 2, replace=False)
            neg_utterance = np.random.choice(sp_to_utt[negative_speaker][:5], 1, replace=True)[0]
