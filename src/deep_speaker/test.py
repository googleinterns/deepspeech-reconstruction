...
def eval_model(working_dir: str, model: DeepSpeakerModel):
    ...
    for i, positive_speaker in tqdm(enumerate(speakers_list), desc='test', total=num_speakers):
        ...
        if np.argmax(y_pred[i]) == 0:
            correct_cnt += 1
        if 0 in list(y_pred[i].argsort()[-5:]):
            top_5_cnt += 1
    ...
    return fm, tpr, acc, eer, correct_cnt / len(speakers_list), top_5_cnt / len(speakers_list)
