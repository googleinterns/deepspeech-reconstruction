...
def _load_checkpoint(session, checkpoint_path, allow_drop_layers):
    ...
    reconstructed_var = set(
        v for v in load_vars if
        v.op.name in ['batch_x_reconstructed', 'batch_y_reconstructed', 'batch_y_reconstructed_logits', 'previous_grad_loss', 'multi_step_noise'] or
        any(s in v.op.name for s in ["ExponentialMovingAverage", "unit_vectors_top_k", "unit_vectors_prev", "x_initial", "radii", "sample_", "model_step", "multi_step", "noise"]))
    load_vars -= reconstructed_var
    init_vars |= reconstructed_var
    ...
    for v in sorted(load_vars, key=lambda v: v.op.name):
        log_info('Loading variable from checkpoint: %s (shape: %s)' % (v.op.name, str(v.shape)))
        tensor = ckpt.get_tensor(v.op.name)
        # if tfv1.type(tensor) == tfv1.float32:
        #     tensor = tfv1.cast(tensor, tfv1.float64)
