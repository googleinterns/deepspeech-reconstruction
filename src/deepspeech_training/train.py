...
def calculate_mean_edit_distance_and_loss(iterator, dropout, reuse):
    ...
    return avg_loss, total_loss, non_finite_files
...
def create_optimizer(learning_rate_var):
    if FLAGS.optimizer == 'adam':
        ...
        optimizer = dp_optimizer.DPGradientDescentGaussianOptimizer(
            ...
            noise_multiplier=FLAGS.gradient_noise / FLAGS.gradient_clip_value * np.sqrt(FLAGS.train_batch_size),
        ...
        for i in range(len(Config.available_devices)):
            ...
            with tf.device(device):
                ...
                with tf.name_scope('tower_%d' % i):
                    ...
                    avg_loss, total_loss, non_finite_files = calculate_mean_edit_distance_and_loss(iterator, dropout_rates, reuse=i > 0)
                    ...
                    if FLAGS.optimizer in ['dp-sgd', 'fast-dp-sgd']:
                        gradients = optimizer.compute_gradients(total_loss, tf.trainable_variables())
                    else:
                        gradients = optimizer.compute_gradients(avg_loss)
                    # if FLAGS.gradient_clipping:
                    #     vars = [v for g, v in gradients]
                    #     grads = [g for g, v in gradients]
                    #     grads, _ = tf.clip_by_global_norm(grads, FLAGS.gradient_clip_value)
                    #     grads = [g + tf.random_normal(g.get_shape(), 0., FLAGS.gradient_noise) for g in grads]
                    #     gradients = [(g, v) for g, v in zip(grads, vars)]
...
def train():
    ...
    checkpoint_saver2 = tfv1.train.Saver(max_to_keep=100)
    checkpoint_path = os.path.join(FLAGS.save_checkpoint_dir, 'train')
    checkpoint_path2 = os.path.join(FLAGS.save_checkpoint_dir, 'train2')
    ...
    with tfv1.Session(config=Config.session_config) as session:
        ...
        def run_set(set_name, epoch, init_op, dataset=None):
            ...
            session.run(init_op)
            checkpoint_saver.save(session, checkpoint_path, global_step=0)
            ...
            while True:
                ...
                if current_step % 1000 == 0:
                    checkpoint_saver2.save(session, checkpoint_path2, global_step=current_step)
